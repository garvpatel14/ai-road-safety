        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-2">
            ...
            {isAuthenticated && (
              <button
                onClick={toggleRole}
                title="Click to toggle User/Admin mode for demo"
                className="hidden sm:flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-brand-500/10 text-brand-600 dark:text-brand-400 border border-brand-500/20 hover:bg-brand-500/20 transition"
              >
                <ShieldCheck className="w-3.5 h-3.5" />
                Role: <span className="capitalize underline">{user?.role}</span>
              </button>
            )}
